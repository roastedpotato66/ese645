...
# Heuristics: Map editing_type_id to (self_replace_steps, cross_replace_steps)
# High Self-Replace = Keep original shape strictly (Good for Color, Material)
# Low Self-Replace = Allow shape changes (Good for Style, Background)
ADAPTIVE_SCHEDULES = {
    # 0: Random - Default
    "0": (0.6, 0.4), 
...
def main():
    ...
    for idx, (img_id, item) in enumerate(progress, start=1):
        ...
        # --- Extension START ---
        # Get category ID for the current image
        category_id = str(item['editing_type_id'])
        
        # Look up adaptive params, default to (0.6, 0.4) if missing
        self_steps, cross_steps = ADAPTIVE_SCHEDULES.get(category_id, (0.6, 0.4))
        
        # Dynamically update the editor configuration
        editor.config.self_replace_steps = self_steps
        editor.config.cross_replace_steps = cross_steps
        # --- Extension END ---
        try:
            editor.edit_image(
